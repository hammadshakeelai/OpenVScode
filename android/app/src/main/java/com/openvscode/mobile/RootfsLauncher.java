...
final class RootfsLauncher {
    ...
    static synchronized void start(Context ctx) throws Exception {
        ...
        Log.i(TAG, "  HOME=" + env.get("HOME"));
        running = pb.start();

        // Report how it ends. Without this a process that dies immediately is
        // indistinguishable from one that started fine, which cost real time:
        // the log file was created and stayed empty, and nothing said why.
        final Process p = running;
        new Thread(() -> {
            try {
                int rc = p.waitFor();
                Log.w(TAG, "IDE process exited with code " + rc
                        + (rc == 139 ? " (SIGSEGV)" : rc == 127 ? " (command not found)"
    ...
    static Map<String, String> baseEnvironment(String root, String home) {
        ...
        env.put("PATH", root + "/usr/local/sbin:" + root + "/usr/local/bin:"
                ...
                + root + "/sbin:" + root + "/bin");
        // Deliberately NO LD_LIBRARY_PATH. Android's linker honours it too, and
        // the rootfs lib directory holds a file named libc.so that is a GNU ld
        // script rather than an ELF — so setting it here killed every bionic
        // binary the app launched, at exec time, with
        //   CANNOT LINK EXECUTABLE "/system/bin/sh": ... bad ELF magic: 2f2a2047
        // Library lookup belongs in each binary's RPATH, which the image build
        // sets and which nothing else can see.
